...
class BmiHBV(Bmi):
    ...
    def initialize(self, par, forcing, rr):
    ...
    def HBVMod(self,Param, Prec, Evap, Si, Su, Sf, Ss, area, Qrr):
        ...
        Qt = Qtotdt*area/3.6 + Qrr
        
        #Offset Q
        Weigths=Weigfun(Tlag)
        Qm = np.convolve(Qt,Weigths)
    ...
    def update(self):
        Qm, self.Si, self.Su, self.Sf, self.Ss = self.HBVMod(
            ...
            self.Ss,
            self.area,
            self.rr.iloc[self.get_current_time()]
        )
